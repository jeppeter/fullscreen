...
HRESULT CD3DEnumeration::EnumerateDevices( CD3DEnumAdapterInfo* pAdapterInfo, CGrowableArray<D3DFORMAT>* pAdapterFormatList )
{
    ...
    for( UINT iDeviceType = 0; iDeviceType < devTypeArrayCount; iDeviceType++ )
    {
        ...
        IDirect3DDevice9 *pDevice = NULL,*pAno=NULL;
        IGraphBuilder * g_pGraph = NULL;
        static int stInitco=0;
        ...
        hr = pDevice->QueryInterface(IID_IDirect3DDevice9,(void**)&pAno);
        if (FAILED(hr))
        {
            DEBUG_INFO("interface query failed\n");
        }
        else
        {
            DEBUG_INFO("interface query succ %p device(%p)\n",pAno,pDevice);
            if (pAno)
            {
                pAno->Release();
            }
        ...
        if (!FAILED(hr))
        {
            hr = g_pGraph->QueryInterface(IID_IDirect3DDevice9,(LPVOID *) &pAno);
            if (FAILED(hr))
            {
                DEBUG_INFO("interface query failed 0x%08x\n",hr);
            }
            else
            {
                DEBUG_INFO("interface query succ %p device(%p)\n",pAno,pDevice);
        ...
        {
            DEBUG_INFO("graph failed 0x%08x\n",hr);
        }
        //DEBUG_BUFFER(pDevice,sizeof(*pDevice));
